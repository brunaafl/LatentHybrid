...
def gen_slice_EEGNet_normtest(n_chans, n_classes, input_window_samples, config, start=0, end=19, drop_prob=0.5, norm=nn.BatchNorm2d):
...
def gen_slice_EEGNet(n_chans, n_classes, input_window_samples, config, start=0, end=19, drop_prob=0.5, remove_bn=True, norm=None):
...
class HybridModel(nn.Module):
	def __init__(self, num_models, model_type, n_chans, n_classes, input_window_samples, config=None, freeze=None, args=None):
		...
		self.shared_modules = model_gen[self.model_type][0](n_chans, n_classes, input_window_samples, config, start=model_gen[self.model_type][2], norm=norms[self.args.sharednorm])
	...
	def init_unique_modules(self, n_chans, n_classes, input_window_samples):
		unique_head = model_gen[self.model_type][0](n_chans, n_classes, input_window_samples, self.config, end=model_gen[self.model_type][1], norm=norms[self.args.uniquenorm])
		return unique_head
...
def define_hybrid_clf(model, config, experiment_name):
	...
	clf = HybridClassifier(
		...
		callbacks=[EarlyStopping(monitor='valid_loss', patience=patience),
				   ...
				   Checkpoint(monitor="valid_loss_best", load_best=True, dirname=f"/workspace/params/temptrain-{experiment_name}", f_params="params.pt"),
				   lrscheduler,
...
class HybridAggregateTransform(BaseEstimator, TransformerMixin):
	...
	def transform(self, X, y=None):
		...
		for trial_i in range(n_trials_per_subject):
			...
			raw = mne.io.RawArray(np.vstack(trial), info) * 1e6
	...
	def evaluate(self, dataset, pipelines, grid_search):
		...
		for train, test in tqdm(cv.split(X, y, groups), total=n_subjects, desc=f"{dataset.code}-CrossSubject",):
			...
			for name, clf in run_pipes.items():
				...
				model = copyclf.fit(X[train], None, Hybrid_adapter__labels=y[train], Hybrid_adapter__subject_groups=groups[train], Hybrid_adapter__info=X[train].info)
				wandb.finish()
				...
				if type(eval_model.unique_modules) != type(nn.Identity()) or list(eval_model.shared_modules.parameters())[0].requires_grad:
